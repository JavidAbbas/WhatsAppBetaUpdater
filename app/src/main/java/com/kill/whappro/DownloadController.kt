...
class DownloadController(private val context: MainActivity, private val url: String, private val version: String) {
    ...
    fun enqueueDownload() {
        ...
        // Configure cancel button and show progress dialog
        val builder = UtilsDialog.showDownloadingDialog(context, version)
        builder.onNegative { _, _ -> cancelDownload() }
        dialog = builder.show()
            ...
            while (downloading) {
                try {
                    ...
                    context.runOnUiThread(Runnable {
                        dialog?.setProgress(dl_progress)
                        downloadProgress.value = dl_progress.toFloat()
                    })
